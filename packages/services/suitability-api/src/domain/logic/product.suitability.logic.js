...
module.exports = class {
  ...
  applyHomeOwnershipStatusScore(house) {
    if (!house) throw Error(`"param 'house' can't be null"`);
    if (house && house.ownership_status.toUpperCase() === OWNERSHIP_STATUS.MORTGAGED) return 1;
  ...
  validateCustomerData(customer) {
    ...
    if (
      !Array.isArray(customer.risk_questions) ||
      customer.risk_questions.length != 3 ||
      customer.risk_questions.some(isNaN)
    ) {
